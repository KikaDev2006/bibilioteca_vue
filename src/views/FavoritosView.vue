  ...
  <Layout>
    <div class="minimal-container">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Libros Favoritos</h1>
      </div>

      <div v-if="loading" class="text-center py-8">
        <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
        <p class="mt-2 text-gray-600">Cargando libros favoritos...</p>
      </div>
      ...
      <div v-else class="text-center py-8">
        <p class="text-gray-600 text-lg mb-4">No tienes libros favoritos aún</p>
        <router-link
          to="/libros"
...
const deleteLibro = async (libroId: number) => {
  ...
  if (libro && confirm(`¿Estás seguro de que quieres eliminar "${libro.nombre}"?`)) {
    try {
      ...
      await fetchFavoritos() // Recargar la lista
    } catch (error) {
